<!--
============================================================================
JAMIA HAMDARD - CDOE - MCA (ONLINE) DISSERTATION
HOW TO USE THIS FILE
============================================================================
1. This is the full draft of your dissertation in Markdown. Open it in MS Word
   (File > Open, choose this .md) OR paste section-by-section into Word.

2. APPLY THESE FORMAT SETTINGS IN WORD (mandatory per CDOE 2026 guidelines):
   - Font:            Times New Roman, 12 pt
   - Line spacing:    1.5 (set References section to 1.0)
   - Alignment:       Justified
   - Margins:         Left 1.5 inch ; Right / Top / Bottom 1 inch
   - Page numbers:    Bottom-right corner
   - Headings:        Use Word Heading 1 / Heading 2 styles so the Table of
                      Contents auto-generates (References > Insert TOC)
   - Final file:      One PDF, OCR / searchable, signed by you and your guide
   - Plagiarism:      Must be < 15%. Re-word any pasted text in your own words.

3. PLACEHOLDERS in [SQUARE BRACKETS] = you must fill these (dates, figures,
   screenshots, signatures, numbers from your real test runs).

4. CITATIONS: Verify EVERY reference before submission. The guideline warns
   against AI-generated / fake citations. Foundational refs (Microsoft, Angular,
   RFC 7519, etc.) are real and verifiable. You MUST expand the list to 50 real
   references (2015-2025) from IEEE Xplore / Google Scholar and confirm each one.

5. FIGURES: Wherever you see [INSERT FIGURE ...], paste an actual screenshot or
   diagram. These fill pages and are expected by evaluators.
============================================================================
-->
...
The system delivers four functional capabilities: a consolidated **dashboard** that summarizes total bills, quantities, and amounts for sales and purchases over a chosen date range and company selection, with click-to-drill-down transaction detail; a three-state **jobber alteration tracker** (Pending, Received, Delivered) with server-side search and pagination; a **company management** module; and a **customer outstanding** reporting module. The backend follows a layered Controller–Service–Repository architecture and applies cross-cutting middleware for global exception handling, request timing, response compression, and caching.
...
3. **RQ3:** Can the informal jobber alteration workflow be modelled as a structured, queryable three-state process (Pending → Received → Delivered) within the same system?
...
The businesses studied — small garment shops, jewellery showrooms, and multi-branch retail outlets — consistently exhibited the following problems:
...
- **O4.** Build a jobber alteration module with three states — Pending, Received, Delivered — including server-side search (by jobber name, item, category) and pagination.
...
**Table 3.1 — Comparison of existing systems with the proposed system**
...
**Token-based authentication and JWT.** The JSON Web Token standard [4] defines a compact, URL-safe, self-contained token format that carries claims between parties. JWTs are widely adopted for stateless authentication in RESTful systems because the server need not maintain session state. *(Expand with 2015–2025 IEEE/ACM papers on "JWT security", "stateless authentication", "token-based access control web API".)*

**Password hashing.** Provos and Mazières [5] introduced BCrypt, an adaptive hash function whose cost factor can be increased over time to remain resistant to brute-force attacks. Adaptive hashing is recommended for password storage in modern applications. *(Expand with recent surveys on "password storage", "adaptive hashing", "credential security".)*

**RESTful architecture.** Fielding's dissertation [6] established the REST architectural style — statelessness, uniform interface, resource orientation — which underpins modern Web APIs. *(Expand with 2015–2025 papers on "REST API design best practices", "API performance", "microservice API design".)*

**Multi-tenant data isolation.** Multi-tenant SaaS literature distinguishes isolation strategies (separate database, separate schema, shared schema with a discriminator column). The proposed system uses a **shared-schema, discriminator-based** approach in which a `ClientId` / `UserGroupId` constrains every query [7]. *(Expand with IEEE papers 2015–2025 on "multi-tenant data isolation", "SaaS tenant isolation", "row-level security".)*

**Single Page Applications and component frameworks.** Modern SPA frameworks (Angular, React) and component libraries (Angular Material) enable responsive, app-like web clients [2][3]. *(Expand with 2015–2025 papers on "single page application performance", "Angular framework", "responsive web design".)*

**.NET and EF Core data access.** ASP.NET Core and Entity Framework Core provide a cross-platform, high-performance server stack with an ORM for relational data access [1]. *(Expand with 2015–2025 papers/benchmarks on ".NET Core performance", "ORM performance EF Core".)*
...
| NFR5 (Maintainability) | Layered Controller–Service–Repository design with interfaces and dependency injection. |
...
The system follows a classic two-tier client–server model with a layered backend.

[INSERT FIGURE 5.1 — High-level architecture: Browser/Android (Angular SPA) ⇄ HTTPS/JSON ⇄ Nginx (SSL) ⇄ ASP.NET Core 8 API ⇄ EF Core ⇄ SQL Server]

The backend uses a **Controller → Service → Repository** layering, with dependency injection wiring interfaces to implementations (registered in `Program.cs`):
...
[INSERT FIGURE 5.2 — Layered backend architecture]
...
**Table 5.4 — Database tables (entities) summary**
...
- `User.ClientId` → `Company.ClientId` (many users to one company).
- `Purchase.PurClientId` → `Company.ClientId` (many purchases to one company).

[INSERT FIGURE 5.3 — ER diagram showing Users, Companies, purchase, PurchaseTrnSummary, CloudCustomerOutstanding and their relationships via ClientId]
...
Exposes three endpoints — `pending`, `received`, `delivered` — that map to a status enum and query the `PurchaseTrnSummary` item lines. Supports filtering by jobber name, item, and category, with server-side pagination (default page size 20, maximum 200).
...
[INSERT FIGURE 5.7 — Login/JWT sequence: Client → POST /User/login → UserService verifies BCrypt → JwtService issues token → Client stores token → subsequent requests send Bearer token]
...
[INSERT FIGURE 5.8 — Dashboard summary sequence: Client → GET /Dashboard/summary (Bearer) → DashboardService resolves allowed clientIds → PurchaseRepository queries by date+type+clientIds → aggregation → response]

## 5.6 Implementation Details

### 5.6.1 Backend — JWT Issuance
...
### 5.6.2 Backend — Credential Verification (BCrypt)
...
### 5.6.3 Backend — Multi-Company Aggregation
...
### 5.6.4 Backend — Allowed Client Resolution (Isolation)
...
### 5.6.6 Frontend — Dashboard and Jobber Calls
...
1. **ExceptionHandlingMiddleware** — global error handling for consistent error responses.
2. **Response compression** and **response caching** — performance.
3. **RequestTimingMiddleware** — logs/measures request duration.
4. **Swagger/OpenAPI** — interactive API documentation.
5. **CORS** — restricted to the known frontend origins (production domain and `localhost:4200`).
6. **Authentication** and **Authorization** — JWT bearer validation.
...
[INSERT FIGURE 5.9 — Deployment topology: Internet → Nginx (443/SSL) → Kestrel (5050) ASP.NET Core → SQL Server; static Angular served over HTTPS]
...
- [INSERT FIGURE 6.1 — Sign In screen with form validation]
- [INSERT FIGURE 6.2 — Dashboard: company dropdown, date-range shortcuts, Sales & Purchase summary cards]
- [INSERT FIGURE 6.3 — Drill-down dialog showing the transaction list behind a summary card]
- [INSERT FIGURE 6.4 — Jobber Alteration screen with Pending/Received/Delivered tabs, search filters and paginated table]
- [INSERT FIGURE 6.5 — Customer Outstanding view]
...
[INSERT FIGURE 6.6 — Swagger UI / Postman showing a successful summary response and a 403 isolation response]
...
Compared with existing tools (Table 3.1), the system uniquely combines multi-company consolidation **and** jobber tracking in one self-hosted application, at no per-company subscription cost — the principal gap identified in the literature review.

<div style="page-break-after: always;"></div>

# Chapter 7: Summary and Conclusions

This dissertation set out to solve a concrete, recurring problem for multi-branch SME retailers: the inability to view consolidated, secure, location-independent business figures, and the lack of structured tracking for the jobber alteration workflow. A two-tier web application was designed and implemented — an ASP.NET Core 8 REST API with Entity Framework Core and SQL Server on the server side, and an Angular 19 SPA (with an optional Capacitor Android build) on the client side — secured with JWT authentication and BCrypt password hashing.

The central technical contribution is a uniform **multi-company data-isolation mechanism**: tenant identifiers (`ClientId`, `UserGroupId`) are embedded as JWT claims and every query is constrained to the caller's permitted companies. On top of this foundation, the system delivers a consolidated sales/purchase dashboard with drill-down, a three-state jobber alteration tracker with search and pagination, company management, and a consolidated customer-outstanding report. The backend's layered Controller–Service–Repository design, together with cross-cutting middleware for error handling, timing, compression, and caching, yields a maintainable and reasonably performant system that was deployed behind an HTTPS reverse proxy.

**Practical implications and recommendations.** The system measurably reduces the effort required for an owner to obtain consolidated multi-company figures — from manually opening several desktop applications to a single secure web view accessible from any device. It is recommended as a deployable prototype for the target SME segment, and as a reusable template (modular architecture) onto which further modules can be added.

In conclusion, the project achieved its stated objectives (O1–O7) and answered its research questions (RQ1–RQ4), demonstrating that an integrated, domain-specific, multi-company retail operations dashboard is both feasible and practically valuable.
...
> Format in IEEE style, 1.0 line spacing. The CDOE guideline requires **at least 50 references (2015–2025)**. The entries below are foundational and verifiable; you must add further peer-reviewed IEEE/ACM journal and conference papers (2015–2025) on the italicized topics from Chapter 3 to reach 50, and verify every entry. Do not include unverified citations.
...
[3] Google, "Angular Material — UI Component Library." [Online]. Available: https://material.angular.io

[4] M. Jones, J. Bradley, and N. Sakimura, "JSON Web Token (JWT)," RFC 7519, IETF, May 2015.

[5] N. Provos and D. Mazières, "A Future-Adaptable Password Scheme," in Proc. USENIX Annual Technical Conference, 1999.
...
[8] Zoho Corp. / Intuit, "Zoho Books and QuickBooks Online — product documentation." [Online]. (Studied during planning.)
...
<!-- [11]–[50] ADD verified peer-reviewed references (2015–2025) here, e.g. on:
...
**Appendix A — Sample API Request/Response (Dashboard Summary)**
[INSERT: example request URL with query params and a trimmed JSON response.]

**Appendix B — Database Schema Script**
...
**Appendix D — Key Source Code Listings**
[INSERT: selected listings — JwtService, DashboardService aggregation, AuthService — if required by your evaluator.]

**Appendix E — Research Publication / Conference Presentation Details (if any)**
